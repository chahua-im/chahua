...
async fn list_pins(
    ...
    let enriched = attach_metadata(conn, msgs, &state.media, state.users.as_ref(), uid).await;
...
async fn create_pin(
    ...
    let enriched = attach_metadata(conn, vec![msg], &state.media, state.users.as_ref(), uid).await;
