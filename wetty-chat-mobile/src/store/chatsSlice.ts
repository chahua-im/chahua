...
/**
 * Whether a live unread-badge count stays authoritative over the incoming
 * server snapshot: only when the snapshot hit the badge cap (so its count may
 * be truncated) and the live counter is even higher is the live value fresher.
 */
function keepLiveBadgeCount(snapshotCount: number, liveCount: number | undefined): boolean {
  return liveCount != null && snapshotCount >= UNREAD_BADGE_COUNT_CAP && liveCount > snapshotCount;
}
...
function reconcileAuthoritativeListFields(
  ...
  if (!keepLiveBadgeCount(snapshotUnreadCount, liveUnreadCount)) {
  ...
  for (const fields of [
    {
      ...
      status: 'unreadMentionIdsStatus',
      snapshot: snapshotUnreadMentions,
    },
    {
      count: 'unreadReactions',
      ids: 'unreadReactionIds',
      status: 'unreadReactionIdsStatus',
      snapshot: snapshotUnreadReactions,
    },
  ] as const) {
    if (!keepLiveBadgeCount(fields.snapshot, entry.liveProjection[fields.count])) {
      delete entry.liveProjection[fields.count];
      delete entry.liveProjection[fields.ids];
      entry.liveProjection[fields.status] = 'idle';
    }
  }
