...
const MountedRowContent = memo(function MountedRowContent({
  row,
  renderRow,
  highlighted,
  hiddenDate,
  ...
  return (
    <div
      className={[
        styles.rowContent,
        highlighted ? styles.rowContentHighlighted : null,
        hiddenDate ? styles.dateRowContentHidden : null,
      ]
        .filter(Boolean)
        .join(' ')}
    >
...
export function ChatVirtualScroll({
  ...
  // P1-2 (Option C): cached visible anchor, refreshed every rAF by
  // updateLastFullyVisibleMessage (reuses its existing getBoundingClientRect
  // loop — zero new measurements). Render-body prepend/append/delete reads this
  // instead of calling captureVisibleAnchor() (which forces sync layout in
  // render). Falls back to captureVisibleAnchor() only when no group was visible
  // in the last frame.
  const lastVisibleAnchorRef = useRef<{ key: string; offsetTop: number } | null>(null);
  ...
  // P2/方案C: accumulate height-preserve deltas from mounted-row ResizeObserver
  // callbacks and apply them in a coalesced rAF instead of synchronously inside
  // each RO callback. Prepend renders new rows with estimated heights; RO fires
  // async (post-paint) to report real heights — a sync scrollTop write here would
  // produce a visible second correction. Coalescing into the next frame merges it
  // with the first correction's paint whenever possible. Mirrors the P0 rAF pattern.
  const pendingHeightPreserveRef = useRef(0);
  const heightPreserveRafRef = useRef<number | null>(null);
  ...
  const updateLastFullyVisibleMessage = useCallback(() => {
    ...
    let nextAnchor: { key: string; offsetTop: number } | null = null;
    if (mounted) {
      ...
      for (let index = mounted.end; index >= mounted.start; index -= 1) {
        ...
        if (row.type === 'group') {
          ...
          if (partiallyVisible) {
            ...
            nextAnchor = { key: row.key, offsetTop: roundScrollValue(rowRect.top - containerRect.top) };
          }
    ...
    // Refresh the cached visible anchor (Option C) — render-body
    // prepend/append/delete reads this instead of forcing sync layout.
    lastVisibleAnchorRef.current = nextAnchor;
  ...
  const handleMountedMeasure = useCallback(
    (key: string, height: number) => {
      ...
      if (preserveContribution !== 0) {
        ...
        // 方案C: defer the scrollTop correction into a coalesced rAF instead
        // of writing synchronously inside this RO callback (which fires post-
        // paint of the estimated-height frame). Accumulate deltas across rows
        // that resize in the same batch and apply once next frame — merges with
        // the first correction's paint whenever possible, eliminating the visible
        // "second correction" jump during prepend.
        if (preserveContribution !== 0) {
          pendingHeightPreserveRef.current += preserveContribution;
          if (heightPreserveRafRef.current == null) {
            heightPreserveRafRef.current = requestAnimationFrame(() => {
              heightPreserveRafRef.current = null;
              const pending = pendingHeightPreserveRef.current;
              pendingHeightPreserveRef.current = 0;
              const container = containerRef.current;
              if (!container || pending === 0) return;
              const target = container.scrollTop + pending;
              if (hasMeaningfulScrollDelta(container.scrollTop, target)) {
                container.scrollTop = target;
              }
            });
          }
  ...
  const handleScroll = useCallback(
    (e: UIEvent<HTMLDivElement>) => {
      // Fold the user-vs-programmatic activity detection (previously a second
      // native addEventListener('scroll')) into this single scroll handler.
      // See plan P0-3.
      ...
      // Windowing + visibility reads are batched into the rAF below (plan P0-1/P0-2).

      if (scrollRafRef.current == null) {
        scrollRafRef.current = requestAnimationFrame(() => {
          scrollRafRef.current = null;
          maybeUpdateMountedForScroll();
          // Batch the layout-thrashing reads (getBoundingClientRect loops /
          // scrollHeight reads) into the same rAF as windowing — once per frame
          // instead of once per scroll event. Mirrors Telegram Web's `fasterdom`
          // approach (measure/mutate coalesced on a single RAF). See plan P0-1/P0-2.
          updateAtBottom();
          updateLastFullyVisibleMessage();
        });
      }
  ...
  useEffect(() => {
    return () => {
      ...
      if (heightPreserveRafRef.current != null) cancelAnimationFrame(heightPreserveRafRef.current);
  ...
  if (rowKeys !== adjustPrevKeysRef.current) {
    if (adjustPrevKeysRef.current.length > 0) {
      ...
      if (mutation === 'prepend') {
        // Capture the anchor with REAL getBoundingClientRect here in render
        // (pre-commit, current frame) — mirrors Telegram Web's useSyncEffect
        // capture. lastVisibleAnchorRef is up to one frame stale (written by the
        // previous rAF); during fast scroll-to-top it can reference a row that
        // already left the viewport, causing restoreAnchorOffset to miscompensate
        // → visible jump. Prepend is low-frequency (history load), so the render-
        // phase layout read is acceptable here (not on the fling hot path).
      ...
      mountedRows.push(
        <MeasuredRow key={row.key} rowKey={row.key} onMeasure={handleMountedMeasure} registerRow={registerRow}>
          <MountedRowContent
            ...
            hiddenDate={row.key === hiddenDateRowKey}
          />
        </MeasuredRow>,
      );
    }
