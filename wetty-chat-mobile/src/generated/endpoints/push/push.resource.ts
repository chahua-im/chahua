 ...
 */
import type { GetSubscriptionStatusParams, SubscriptionStatusResponse, VapidPublicKeyResponse } from '../../models';
 ...
 */
export function getSubscriptionStatusResource(
  params: Signal<GetSubscriptionStatusParams> | undefined,
  options: OrvalHttpResourceOptions<SubscriptionStatusResponse, unknown> & {
    defaultValue: NoInfer<SubscriptionStatusResponse>;
  },
): HttpResourceRef<SubscriptionStatusResponse>;
export function getSubscriptionStatusResource(
  params?: Signal<GetSubscriptionStatusParams>,
  options?: OrvalHttpResourceOptions<SubscriptionStatusResponse, unknown>,
): HttpResourceRef<SubscriptionStatusResponse | undefined>;
export function getSubscriptionStatusResource(
  params?: Signal<GetSubscriptionStatusParams>,
  options?: OrvalHttpResourceOptions<SubscriptionStatusResponse, unknown>,
): HttpResourceRef<SubscriptionStatusResponse | undefined> {
  const baseUrl = options?.injector ? options.injector.get(CHAHUA_BASE_URL) : inject(CHAHUA_BASE_URL);
  return httpResource<SubscriptionStatusResponse>(() => {
    const request = {
      url: `${baseUrl}/push/subscription-status`,
 ...
 * @remarks httpResource is available in Angular 19.2 and later.
 */
export function getVapidPublicKeyResource(
  options: OrvalHttpResourceOptions<VapidPublicKeyResponse, unknown> & {
    defaultValue: NoInfer<VapidPublicKeyResponse>;
  },
): HttpResourceRef<VapidPublicKeyResponse>;
export function getVapidPublicKeyResource(
  options?: OrvalHttpResourceOptions<VapidPublicKeyResponse, unknown>,
): HttpResourceRef<VapidPublicKeyResponse | undefined>;
export function getVapidPublicKeyResource(
  options?: OrvalHttpResourceOptions<VapidPublicKeyResponse, unknown>,
): HttpResourceRef<VapidPublicKeyResponse | undefined> {
  const baseUrl = options?.injector ? options.injector.get(CHAHUA_BASE_URL) : inject(CHAHUA_BASE_URL);
  return httpResource<VapidPublicKeyResponse>(
    () => applyOrvalRequestExtension(`${baseUrl}/push/vapid-public-key`, options),
    options,
  );
}

export type GetSubscriptionStatusResourceResult = NonNullable<SubscriptionStatusResponse>;
export type GetVapidPublicKeyResourceResult = NonNullable<VapidPublicKeyResponse>;
