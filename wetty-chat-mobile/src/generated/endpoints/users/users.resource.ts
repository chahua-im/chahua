 ...
 */
import type { AuthTokenResponse, GetUserSearchParams, MeResponse, SearchUsersResponse } from '../../models';
...
export function getAuthTokenResource(
  options: OrvalHttpResourceOptions<AuthTokenResponse, unknown> & { defaultValue: NoInfer<AuthTokenResponse> },
): HttpResourceRef<AuthTokenResponse>;
export function getAuthTokenResource(
  options?: OrvalHttpResourceOptions<AuthTokenResponse, unknown>,
): HttpResourceRef<AuthTokenResponse | undefined>;
export function getAuthTokenResource(
  options?: OrvalHttpResourceOptions<AuthTokenResponse, unknown>,
): HttpResourceRef<AuthTokenResponse | undefined> {
  const baseUrl = options?.injector ? options.injector.get(CHAHUA_BASE_URL) : inject(CHAHUA_BASE_URL);
  return httpResource<AuthTokenResponse>(
    () => applyOrvalRequestExtension(`${baseUrl}/users/auth-token`, options),
    options,
  );
}
...
export function getMeResource(
  ...
  const baseUrl = options?.injector ? options.injector.get(CHAHUA_BASE_URL) : inject(CHAHUA_BASE_URL);
  return httpResource<MeResponse>(() => applyOrvalRequestExtension(`${baseUrl}/users/me`, options), options);
}

/**
 * @remarks httpResource is available in Angular 19.2 and later.
 */
export function getUserSearchResource(
  params: Signal<GetUserSearchParams> | undefined,
  options: OrvalHttpResourceOptions<SearchUsersResponse, unknown> & { defaultValue: NoInfer<SearchUsersResponse> },
): HttpResourceRef<SearchUsersResponse>;
export function getUserSearchResource(
  params?: Signal<GetUserSearchParams>,
  options?: OrvalHttpResourceOptions<SearchUsersResponse, unknown>,
): HttpResourceRef<SearchUsersResponse | undefined>;
export function getUserSearchResource(
  params?: Signal<GetUserSearchParams>,
  options?: OrvalHttpResourceOptions<SearchUsersResponse, unknown>,
): HttpResourceRef<SearchUsersResponse | undefined> {
  const baseUrl = options?.injector ? options.injector.get(CHAHUA_BASE_URL) : inject(CHAHUA_BASE_URL);
  return httpResource<SearchUsersResponse>(() => {
    const request = {
...
export type GetAuthTokenResourceResult = NonNullable<AuthTokenResponse>;
export type GetMeResourceResult = NonNullable<MeResponse>;
export type GetUserSearchResourceResult = NonNullable<SearchUsersResponse>;
