...
import type { UnreadIdCacheStatus } from '@/store/unreadIdCache';
import { pickNextUnreadId } from '@/store/unreadIdCache';

export interface UnreadJumperSnapshot {
  unreadCount: number;
  ids: string[];
  status: UnreadIdCacheStatus;
}
...
export interface UseUnreadIdJumperArgs {
  ...
  setStatus: (status: UnreadIdCacheStatus) => void;
