  ...
  async toggleReaction(message: MessageResponse, emoji: string): Promise<void> {
    ...
    const reactions = message.reactions.map((reaction) =>
      reaction.emoji === emoji
        ? { ...reaction, count: reaction.count + (reacted ? 1 : -1), reactedByMe: reacted }
        : reaction,
    );
    if (!existing) reactions.push({ emoji, count: 1, reactedByMe: true });
    // Publish the personal selection now; later broadcasts replace totals and preserve it.
    this.realtime.acceptChange({
