...
组件按界面职责组织。业务列表和聊天资料来自共享 Store；对话消息来自页面提供的 ConversationStore；滚动、菜单、表单与按钮等待状态由相应组件持有。固定文案和展示配置位于 HTML 模板。
...
flowchart TD
  ...
  OUTLET --> COLL[ConversationCollectionPage]
...
| ------------ | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| App          | 浏览器 popstate、分栏可见性、路由栈事件                                                      | 登录 loading/error、分栏可见性、浏览器是否已播放返回动画                              | SessionStore 初始化身份；Router 协调 Ionic 默认页面动画；装配分栏和设置弹窗                                                                                     |
| ChatListPage | 列表路由；Ionic 进入/离开事件                                                                | listActive                                                                            | 为移动列表提供 active，桌面主区域显示聊天占位页；离开时驱动子查询清理                                                                                           |
| ChatList     | 输入 active；读取当前路由；点击分类、设置、刷新和续页                                        | 最后一次列表选择、首次展示 ready、下拉 refreshing；其他行、排序、加载和计数为派生状态 | ChatListStore 提供查询与操作，ChatStore 补充资料，DraftStore 提供草稿，Preferences 决定话题展示，SessionStore 提供当前用户，ConversationNavigation 定位当前对话 |
...
ChatList 在桌面侧栏和路由页面中可以有不同实例，共享 Store 中的查询。组件选择 `chats(archived)`、`threads(archived)`、`friendRequests(history)` 和归档计数查询；active 控制其消费者生命周期。
...
| 组件                       | 路由输入                                                     | 局部状态                                                                                                                            | 服务与子组件                                                                                                                                                                          |
| -------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ConversationPage           | id、threadId、message、reply；十进制路由 ID 转成 SnowflakeID | 页面进入上下文、滚动/触摸活动、定位目标、固定未读边界、底部状态、输入与回复预览、发送状态、话题按钮状态、当前置顶选择和置顶读取错误 | 提供 ConversationStore；消费 ChatListStore 的读状态与订阅、ChatStore 的标题、DraftStore、Preferences、SessionStore、Connection 和 ConversationNavigation；驱动 Message 与 MessageMenu |
| ConversationCollectionPage | collection、id、threadId                                     | 活跃版本、收藏列表与续页游标、loading/failed、removingSavedId/removeFailed                                                          | 提供独立 ConversationStore；置顶从该 Store 获取，收藏从 SavedMessagesService 获取；ChatStore 确认权限；复用 Message，置顶分支使用 MessageMenu                                         |
...
ConversationCollectionPage 的收藏数据是快照，`savedMessageContent` 仅映射 Message 需要的展示字段。收藏使用 `interactive=false`，来源、日期、查看原消息和取消收藏按钮由页面展示。置顶行保留回复、定位、话题和消息菜单交互。
...
| MessageMenu        | chatId、threadId、canReply、showAllAvatars                                                                       | reply(message)、openThread(rootId)；open(selection)、reactTo(message, emoji)、reset()；只读 busy | 所选消息 ID/锚点、确认意图、最近表情、提示、忙碌状态、表情面板与菜单位置；提供 MessageActions，使用页面 ConversationStore、ChatStore、SessionStore 和 Router |
| EmojiPicker        | 模板中的中文静态配置                                                                                             | chosen(emoji)                                                                                    | 封装 emoji-picker-element；代码和中文搜索数据按需加载                                                                                                        |

MessageMenu 从本页消息区间或置顶列表中解析选中的消息，因此编辑、撤回和表态更新可以反映到已打开的菜单。selection 只保存定位所需的 ID、元素、矩形和分组信息；确认框保存具体消息与用户确认的置顶意图。

菜单内的职责分工：

- 复制和复制链接直接在点击处理期间发起 Clipboard 写入，以保留 Safari 的用户激活状态。
- 回复与打开话题通过输出交给父页面。
- 置顶和取消置顶先确认，再调用 ConversationStore.setPinned。
