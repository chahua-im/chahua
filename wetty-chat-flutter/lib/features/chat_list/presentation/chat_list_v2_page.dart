...
class _ChatListV2PageState extends ConsumerState<ChatListV2Page> {
  ...
  void _onScroll() {
    ...
    if (activeTab == ChatListTab.threads) {
      final provider = threadListV2ViewModelProvider(widget.scope);
    ...
    if (activeTab == ChatListTab.all) {
      final provider = allListV2ViewModelProvider(widget.scope);
      final allState = ref.read(provider);
      if (allState.isLoadingMore) {
        return;
      }
      ref.read(provider.notifier).loadMoreAll();
    }
  }
  ...
  Future<void> _refreshActiveTab(ChatListTab activeTab) {
    return switch (activeTab) {
      ...
      ChatListTab.all =>
        ref
            .read(allListV2ViewModelProvider(widget.scope).notifier)
            .refreshAll(),
    };
  }
