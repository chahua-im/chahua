 ...
 */
import type { GetGroupsParams, GroupInfoResponse, ListGroupsResponse, SnowflakeID } from '../../models';
 ...
 */
export function getGroupsResource(
  params: Signal<GetGroupsParams> | undefined,
  options: OrvalHttpResourceOptions<ListGroupsResponse, unknown> & {
    defaultValue: NoInfer<ListGroupsResponse>;
  },
): HttpResourceRef<ListGroupsResponse>;
export function getGroupsResource(
  params?: Signal<GetGroupsParams>,
  options?: OrvalHttpResourceOptions<ListGroupsResponse, unknown>,
): HttpResourceRef<ListGroupsResponse | undefined>;
export function getGroupsResource(
  params?: Signal<GetGroupsParams>,
  options?: OrvalHttpResourceOptions<ListGroupsResponse, unknown>,
): HttpResourceRef<ListGroupsResponse | undefined> {
  const baseUrl = options?.injector ? options.injector.get(CHAHUA_BASE_URL) : inject(CHAHUA_BASE_URL);
  return httpResource<ListGroupsResponse>(() => {
    const request = {
      url: `${baseUrl}/group`,
 ...
 * @remarks httpResource is available in Angular 19.2 and later.
 */
export function getGroupResource(
  chatId: Signal<SnowflakeID>,
  options: OrvalHttpResourceOptions<GroupInfoResponse, unknown> & {
    defaultValue: NoInfer<GroupInfoResponse>;
  },
): HttpResourceRef<GroupInfoResponse>;
export function getGroupResource(
  chatId: Signal<SnowflakeID>,
  options?: OrvalHttpResourceOptions<GroupInfoResponse, unknown>,
): HttpResourceRef<GroupInfoResponse | undefined>;
export function getGroupResource(
  chatId: Signal<SnowflakeID>,
  options?: OrvalHttpResourceOptions<GroupInfoResponse, unknown>,
): HttpResourceRef<GroupInfoResponse | undefined> {
  const baseUrl = options?.injector ? options.injector.get(CHAHUA_BASE_URL) : inject(CHAHUA_BASE_URL);
  return httpResource<GroupInfoResponse>(
    () => applyOrvalRequestExtension(`${baseUrl}/group/${decodeId(chatId())}`, options),
    options,
  );
}
export type GetGroupsResourceResult = NonNullable<ListGroupsResponse>;
export type GetGroupResourceResult = NonNullable<GroupInfoResponse>;
/**
