...
资料标题使用三行截断，小字说明使用 caption 字号和行高。管理员的群名称与简介右侧显示铅笔，编辑态在原位置显示文本框与对勾；名称沿用标题字号，编辑时保留原标题占位，输入框覆盖同一区域，切换编辑状态不改变标题高度；简介输入使用正文字号，保存期间对勾替换为 spinner。头像右下角显示 28px 相机角标，上传进度叠在头像内。用户资料的操作按单行排列，侧栏与 modal 共用相同结构。关闭按钮为 44×44px，图标为 28px；会话标题头像为 32px。话题角标沿用旧版比例：主头像的 55% 四舍五入到整数、最小 16px，固定向上/右偏移 2px，2px 外圈不占用头像内部面积。资料头像 88px 时角标内容为 48px。
