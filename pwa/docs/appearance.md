...
资料标题使用三行截断，小字说明使用 caption 字号和行高。用户资料的操作按三列排列，侧栏与 modal 共用相同结构。关闭按钮为 44×44px，图标为 28px；会话标题头像为 32px。话题角标沿用旧版比例：主头像的 55% 四舍五入到整数、最小 16px，固定向上/右偏移 2px，2px 外圈不占用头像内部面积。资料头像 88px 时角标内容为 48px。
