...
聊天列表保留 Ionic `ion-segment` 的选中指示器。内容区使用浏览器 Web Animations，只让当前分类与目标分类按方向横移一页宽，时长 200ms；中间分类不参与。快速切换取消上一段动画，减少动态效果时直接切换。内容区不提供左右手势，toolbar 保持固定。四个分类常驻，各自保留滚动位置，只有当前分类激活读取和续页。归档与好友请求历史使用独立列表内容和 Ionic 返回按钮；单列保留分类路由和浏览器历史，分栏切分类不改变右侧会话。

会话的消息与置顶独立加载。置顶数据即时更新；置顶栏的出现与消失等待手指松开且惯性滚动结束，期间多次变更只展示最终状态。已显示的置顶栏更新内容无需等待。消息区与可视区域的尺寸由 ResizeObserver 监听：最新区间在底部时保持贴底；阅读历史时按可视区域顶部的位移补偿滚动，使置顶栏出现或消失时消息保持原屏幕位置。仅输入栏高度改变时，历史阅读位置保持不变。监听不在每次滚动中运行，不使用定时补滚动。悬浮日期和右下角按钮保持节点常驻，只切换可见性，滚动开始与结束不增删内容槽节点。右下角按钮在首次定位及滚动位置测量完成后才决定显隐，避免数据到达时短暂闪现。
...
资料区横排操作使用 Ionic clear 按钮与主题色浅底：普通操作背景为 primary 的 8%，hover/focused 为 12%，按下为 16%；破坏性操作使用 danger 的 10%/14%/18%。悬停时图标及文字变为对应 shade 色，颜色过渡为 160ms；侧栏与详情弹窗共用。
...
- 气泡内边距为 8px/11px。消息行底部留 4px；同作者连续消息顶部留 4px，新消息组顶部留 8px，因此普通气泡间隔分别为 8px 和 12px。头像距消息区域左右边缘均为 7px，与气泡、桌面回复按钮占用同一网格行并底部对齐，不参与消息行高计算；下方表态及重试按钮占用独立行，不改变头像、尖角和回复按钮的对齐。时间在末行有空间时靠右，空间不足时另起一行；其向下偏移由正文与时间行高之差决定。“已编辑”、时间与发送状态统一使用气泡文字颜色的 70% 不透明度，随亮暗主题及收发气泡配色变化。
...
- 贴纸面板高 260px，网格行高随正方形按钮自然展开，超出面板后纵向滚动；底部包栏固定可见。图片与视频缩略图在 72px 方框内等比居中显示，不裁切。包栏预览使用 24px 图片或静音、循环、内联自动播放的视频；没有可用图片或视频预览时显示默认包图标。
- 输入框文字默认垂直居中，使用正文角色，文字区域与 Ionic 自动增高镜像共用行高。多行输入自动增高至 33vh，超过后仅文字区域纵向滚动，贴纸按钮保持在输入行底部。上下留白由控件高度、边框和行高计算；文字和语音发送飞机均向右偏移 1.5px。
...
- 只有用户发布的 URL 显示下划线；收到的消息链接使用主题蓝色，自己的蓝色气泡内使用白色。系统消息的用户名链接和用户 mention 均不显示下划线。
- 同消息的多张图片/视频按附件顺序拼成相册，最大宽度 360px、间距 2px；两张横图上下排、其他双图并排，三张按首图方向选左一右二或上一下二，四张用四宫格，五至九张按每行两至三张选择均衡布局。宽高比小于 0.5 或大于 2.5 的媒体独占一行，最大高度 360px；其余格子裁切超过 20% 时完整显示并留背景。超过九项只显示九个预览，末格显示 +N，查看器仍包含全部媒体。单图、贴纸、语音和文件沿用各自尺寸框。相册尺寸由元数据与容器宽度确定，边界对齐物理像素，末格吸收舍入余量；缺少尺寸的方形占位在资源加载与上传处理中保持不变。时间仅在末个可见媒体上显示一次，说明文字位于相册下方。
...
NotificationPrompt 使用 Angular 默认的 Emulated 封装，模板内 spinner 的定位和显隐由组件样式负责。Ionic 动态生成的通知弹窗按钮与外框样式放在 `styles.scss`，通过 `ion-alert.notification-prompt` 限定作用范围。
...
| App                    | 384px 聊天列表侧栏与登录期间的空 toolbar、右侧 spinner。                                                                                 |
...
Angular 挂载前的全屏占位使用 toolbar 背景色，跟随深浅主题；挂载后的正文区使用正常页面背景色。登录期间 toolbar 右侧 spinner 复用图标按钮的容器与 `.action-icon` 尺寸。媒体画布缩放尺寸仅由 --media-scale 定义，邀请卡片几何仅由 InviteCard 定义。

资料标题使用三行截断，小字说明使用 caption 字号和行高。管理员的群名称与简介右侧显示铅笔，编辑态在原位置显示文本框与对勾；名称沿用标题字号，编辑时保留原标题占位，输入框覆盖同一区域，切换编辑状态不改变标题高度；简介输入使用正文字号，保存期间对勾替换为 spinner。头像右下角显示 28px 相机角标，上传进度叠在头像内。用户资料的操作按单行排列，侧栏与 modal 共用相同结构。关闭按钮为 44×44px，图标为 28px；会话标题居左并参与 toolbar 弹性布局，按两侧按钮实际宽度收缩；标题头像为 32px，标题内容上下各留 4px，容纳话题角标及外圈，避免被 Ionic 标题的省略裁切截断；主 toolbar 使用 Ionic 内置变量清除重复的上下内边距，保持默认 44px 高度，与聊天列表顶栏一致。话题角标沿用旧版比例：主头像的 55% 四舍五入到整数、最小 16px，固定向上/右偏移 2px，2px 外圈不占用头像内部面积。资料头像 88px 时角标内容为 48px。
...
消息组延伸到视口底部时，固定头像与原头像同为 34px，距发送方一侧边缘 7px；短暂进入视口的新组从组顶开始显露头像。固定容器常驻，不增删 Ionic 默认内容槽，原头像进入可视范围后恢复显示。仅无头像图片的文字占位头像使用名称哈希背景色，明暗模式使用对应颜色；有头像图片时不添加背景色。
