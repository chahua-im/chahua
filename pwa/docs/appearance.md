...
聊天列表使用 `ion-segment`、`ion-segment-view` 和 `ion-segment-content` 的默认内容滑动及选中指示器联动，不自定义位移、时长或曲线。toolbar 保持固定。列表项的选中前景色与 Ionic 实际显示的背景配色同步，路由切换不能使预览正文先变黑。四个分类共用列表页面，各自保留滚动位置，只有当前分类激活读取和续页；归档与好友请求历史使用独立列表内容。单列保留分类路由和浏览器历史，分栏切分类不改变右侧会话。归档与好友请求历史的顶栏使用 Ionic iOS 返回按钮；单列返回导航栈中的上一页，无历史时回到对应分类，分栏仅恢复左侧分类。
...
iOS 根布局以 VisualViewport 的高度和顶部偏移设置 Ionic 已固定的 body，响应 resize/scroll；捏合缩放时交还浏览器布局。可见区域比布局视口矮超过 150px 时暂时清除底部安全区，采用与 Ionic 相同的键盘判断阈值，避免普通浏览器工具栏变化触发。历史阅读补偿只计算应用容器内的顶部变化。消息菜单将视口坐标换算到自身容器，不重复叠加根布局偏移。
...
- 气泡内边距为 6px/11px，连续消息间隔 2px、换作者 5px；消息组上方 2px、下方 3px。头像距消息区域左右边缘均为 7px，保留横向槽位并底部对齐，不参与消息行高计算。时间在末行有空间时靠右，空间不足时另起一行；其向下偏移由正文与时间行高之差决定。
